...
public class MultiplierCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
    ...
    if (args.length < 1 || args.length > 2) {
      MessageUtils.sendMessage(sender, MessageUtils.MAIN_COLOR + "&lMultiplier Management help menu", false);
    ...
    if (args.length == 1) {
      ...
      if (args[0].equalsIgnoreCase("enable")) {
        if (MultiplierHandler.isMultiplierEnabled()) {
          MessageUtils.sendMessage(sender, MessageUtils.ERROR_COLOR + "Multiplier is already enabled.", true);
        } else {
          MessageUtils.sendMessage(sender, MessageUtils.SUCCESS_COLOR + "Multiplier is now enabled.", true);
      ...
      if (args[0].equalsIgnoreCase("disable")) {
        ...
        if (MultiplierHandler.isMultiplierEnabled()) {
          MessageUtils.sendMessage(sender, MessageUtils.SUCCESS_COLOR + "Multiplier is now disabled.", true);

          MultiplierHandler.toggleMultiplier(false);
        } else {
          MessageUtils.sendMessage(sender, MessageUtils.ERROR_COLOR + "Multiplier is already disabled.", true);
        }
    ...
    MessageUtils.sendMessage(sender, MessageUtils.ERROR_COLOR + "Type /mm for command help", true);
    return true;
  }
}
